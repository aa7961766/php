...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $filePath)) {
            // 获取当前脚本所在的目录
            $baseUrl = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . str_replace('/admin/add_article.php', '', $_SERVER['REQUEST_URI']);
            // 转换为绝对路径
    ...
    if (!empty($title) && !empty($content)) {
        try {
            $sql = "INSERT INTO articles (title, content, author_id, category_id, image) VALUES (:title, :content, :author_id, :category_id, :image)";
            ...
            <div class="mb-4">
                ...
                <select id="category_id" name="category_id"
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= $category['id'] ?>"><?= safe_html($category['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="mb-4">
